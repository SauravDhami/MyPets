...
exports.getAllUserOrders = catchAsync(async (req, res, next) => {
    const order = await Order.findOne({ buyer: req.user.id });
    ...
    res.status(200).json({
        ...
        message: 'Your order have been found!!',
        data: order,
    });
});
...
exports.getUserOrder = catchAsync(async (req, res, next) => {
    ...
    if (!order) {
        return next(new AppError('There is no order with given ID!!', 400));
    }
...
exports.createOrder = catchAsync(async (req, res, next) => {
    ...
    if (!order) {
        return next(new AppError('Your order could not be issued!! , Please try again !!', 400));
    }
...
exports.deleteUserOrder = catchAsync(async (req, res, next) => {
    ...
    res.status(200).json({
        status: 'success',
        message: 'Your order has been deleted succesfully!!',
        data: null,
    });
});
